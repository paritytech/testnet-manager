...
from app.lib.network_utils import list_substrate_nodes, list_validators, list_parachains, list_parachain_collators, \
    ...
    set_collator_nodes_keys_on_chain, set_invulnerable_collators
...
@router.post("/collators/{para_id}/set_invulnerables")
async def set_invulnerables(
    para_id: str = Path(description="Parachain ID"),
    address: list[str] = Query(default=[], description="Addresses to be added as invulnerables"),
    node: list[str] = Query(default=[], description="Collator nodes to be added as invulnerables on the parachain")
):
    if node or address:
        asyncio.create_task(set_invulnerable_collators(para_id, node, address))
    return PlainTextResponse('OK')


@router.post("/collators/{para_id}/add_invulnerable")
...
@router.post("/collators/{para_id}/remove_invulnerable")
